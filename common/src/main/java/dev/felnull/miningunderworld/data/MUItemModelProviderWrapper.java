...
public class MUItemModelProviderWrapper extends ItemModelProviderWrapper {
    ...
    public void generateItemModels(ItemModelProviderAccess providerAccess) {
        ...
        StreamSupport.stream(MUItems.ITEMS.spliterator(), false).map(Supplier::get).forEach(item -> {
            if (item instanceof WeatheringItem)//錆びる系のみ手動登録
                return;

            /*if (item instanceof WeatheringItem)//錆び
                if (item instanceof TieredItem)//ツール
                    handheldWeatheringItem(item, providerAccess::handheldFlatItem);
                else//それ以外、アーマー等
                    handheldWeatheringItem(item, providerAccess::basicFlatItem);
            else */
            if (item instanceof TieredItem)//ツール
        ...
        handheldWeatheringItem(providerAccess, MUItems.COPPER_AXE.get());
        handheldWeatheringItem(providerAccess, MUItems.COPPER_SHOVEL.get());
        basicWeatheringItem(providerAccess, MUItems.COPPER_HELMET.get());
        basicWeatheringItem(providerAccess, MUItems.COPPER_CHESTPLATE.get());
        basicWeatheringItem(providerAccess, MUItems.COPPER_LEGGINGS.get());
        basicWeatheringItem(providerAccess, MUItems.COPPER_BOOTS.get());
    }

    private MutableFileModel handheldWeatheringItem(Item item, Function<ResourceLocation, MutableFileModel> modelType) {
    ...
    private MutableFileModel handheldWeatheringItem(ItemModelProviderAccess providerAccess, Item item) {
        ...
        var subModels = subBasicWeatheringItems(providerAccess, BuiltInRegistries.ITEM.getKey(item).getPath());
    ...
    private MutableFileModel basicWeatheringItem(ItemModelProviderAccess providerAccess, Item item) {
        var model = providerAccess.basicFlatItem(item);
        var subModels = subBasicWeatheringItems(providerAccess, BuiltInRegistries.ITEM.getKey(item).getPath());
    ...
    private Map<WeatheringItem.WeatheringState, MutableFileModel> subBasicWeatheringItems(ItemModelProviderAccess providerAccess, String name) {
        return Arrays.stream(WeatheringItem.WeatheringState.values())
                .filter(it -> it != WeatheringItem.WeatheringState.NONE)
                .collect(Collectors.toMap(oxidizingState -> oxidizingState, oxidizingState -> providerAccess.basicFlatItem(modLoc(oxidizingState.getSerializedName() + "_" + name)), (mutableFileModel, mutableFileModel2) -> mutableFileModel, TreeMap::new));
    }
}
